...
def daily_meals():
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('summary',calories=calories, timeFrame=timeFrame, targetCalories=targetCalories, diet=diet, exclude=exclude ))
...
def summary():
    ...
    print(type(initarget))
    print(type(calories))
    if(initarget > calories):
        karbo = "{:.2f}".format((0.65 * calories)/4)
    elif (initarget < calories):
